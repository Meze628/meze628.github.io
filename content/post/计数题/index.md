...
## [P17284 「IXOI R2」想不出来](https://www.luogu.com.cn/problem/P17284)

**挖掘性质+计数 DP**

考虑最终生成的序列有什么性质，发现若最终序列里有一个位置上的数大于 $1$，其左右两边必须是空位，若有一段区间是连续的 $1$，则表明该区间未被操作，因为该区间未被操作，就使得该区间左右两边的操作被隔离开。形式化地，定义前缀和数组 $s_i=\sum_{j=1}^{i} p_j$，则一个序列被认为是可被生成的充要条件是：

> 对于序列中所有两个连续的大于零的数 $p_i, p_{i+1}$，满足 $p_i=p_{i+1}=1, s_i=i$。

考虑 DP，设 $f_{i,j}$ 表示处理到了第 $i$ 个数，第 $i$ 个数是 $0$ 且总和为 $j$ 时的权值和，同样地，$g_{i,j}$ 要求 $p_i=1$，$h_{i,j}$ 要求 $p_i \ge 2$。

对于 $f_{i,j}$，由于第 $i$ 个数贡献的是 $x_i^0=1$，因此有

$$
f_{i,j}=f_{i-1,j}+g_{i-1,j}+h_{i-1,j}
$$

对于 $g_{i,j}$，前一个数只能是 $0$ 或 $1$，当前一个数为 $1$ 时，需要保证 $i=j$，因此有

$$
g_{i,j}=x_if_{i-1,j-1}+[i=j] \cdot x_i g_{i-1,j-1}
$$

对于 $h_{i,j}$，前一个数只能是 $0$，当 $p_i=2$ 时，可以从 $f_{i-1,j-2}$ 转移过来，当 $p_i > 2$ 时，可以从 $h_{i,j-1}$ 转移过来，因此有

$$
h_{i,j}=x_i^2f_{i-1,j-2}+x_ih_{i,j-1}
$$

最后答案就是 $f_{n,n}+g_{n,n}+h_{n,n}$。时间复杂度可以做到 $O(n^2)$ 。
